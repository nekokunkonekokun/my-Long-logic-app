...
if run_btn:
    ...
    with st.spinner("市場のしこり・清算データを解析中..."):
        ...
        if df.empty or len(df) < 10:
            ...
            global_min = float(df['Low'].min())
            global_max = float(df['High'].max())
            ...
            lowest_price = float(recent_df.loc[lowest_idx, 'Low'])
            ...
            if current_position_ratio > 0.75:
                ...
                if has_upper_wall and is_fresh_short:
                    if is_maintaining_high:
                        analysis_text += f"直近で {heavy_sell_price:,.0f}円付近に形成された売りボリュームは、上値を抑え込もうとした『新規の強気ショート』です。価格がこの高値を維持していることは、ショート勢の売り圧力が完全に吸収されている証拠であり、ここを上抜けると彼らの降伏（損切り注文）を巻き込んだ**爆発的な踏み上げ（ショートスクイーズ）**に発展する確率が極めて高くなります。"
                    else:
